...
The most widely used versions of human reference genome are GRCh37 and GRCh38, with the latter being more recent and constantly updated. However, if the data or database used in the experiment is still on version GRCh37, then the same reference genome should be used. It is important to note that these are the **[Ensembl](https://www.ensembl.org/index.html)** reference genomes. Their **[USCS](https://hgdownload.soe.ucsc.edu/downloads.html)** counterparts (hg19 for GRCH37 and hg38 for GRCh38) are equally valid but differ in naming and structure. While in the Ensembl genomes chromosomes are named with only numbers, USCS uses chr1, chr2, etc. Some alignment tools require exact chromosome names, or they will fail or mismatch, so it is essential to choose one format and stick with it for the whole analysis.
