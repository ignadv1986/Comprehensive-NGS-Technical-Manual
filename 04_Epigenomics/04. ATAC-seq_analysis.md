...
However, accounting for the Tn5 shift is critical for applications requiring base-pair resolution, such as TF footprinting or precise motif centering, where the exact coordinates of the Tn5 insertion define the boundaries of protein-DNA interactions. In these cases, the BAM files created with `alignmentSieve --ATACshift` must be used to call peaks in MACS3. Importantly, reads must be treated as single insertions (`-f BAM`) to keep the high resolution.
