...
- **Adapter dimers:** An excess or an incorrect ratio with gDNA of the adapters used in the library preparation, can lead to their dimerization, creating a characteristic sharp peak of low molecular weight on the TapeStation profile. To avoid this, the final concentration of each adapter should be between 0.2 - 0.5 µM, but it's essential to take into account the adapter-to-template ratio: if there is too much adapter relative to very few DNA fragments, the adapters are more likely to find each other (dimerization) than they are to find a rare DNA template, while if not enough is added the reaction will reach adapter starvation before the desired library concentration is achieved.
The presence of adapter dimers can also be a consequence of a too permissive SPRI clean-up step. In this case, a further 0.6x or 0.8x (e.g., 40µL beads to 50µL DNA) AMPure bead cleanup is recommended.
...
- **DNA/RNA degradation:** Poor sample handling before and during library prep can cause genome degradation, leading to a very wide peak that sometimes fuses with the adapter dimer peak, and a virtual gel showing a smeary signal.
- **Over-fragmentation or excessive enzymatic digestion:** Parameters such as sonication intensity or cycle number in case of mechanical fragmentation, and enzyme concentration, and digestion time, in assays in which fragmentation is dependent on enzymatic activity, should be tightly controlled.
- **Suboptimal size selection conditions:** Using too permissive SPRI ratios leads to a less stringent size selection and therefore to wider or smeared distributions.
