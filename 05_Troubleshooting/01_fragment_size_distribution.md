...
- **Adapter dimers:** An excess or an incorrect ratio with gDNA of the adapters used in the library preparation, can lead to their dimerization, creating a characteristic sharp peak of low molecular weight on the TapeStation profile. To avoid this, the final concentration of each adapterer should be between 0.2-0.5 µM, but it's essential to take into account the adapterer-to-template ratio: if there is too much adapter relative to very few DNA fragments, the adapters are more likely to find each other (dimerization) than they are to find a rare DNA template, while if not enough is added the reaction will reach adapter starvation before the desired library concentration is achieved.
